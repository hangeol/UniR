...
from trl.trainer.grpo_config import GRPOConfig
#from RGRPOConfig import GRPOConfig
...
#from rewards import extract_number_answer
from math_grader import grade, extract_answer
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            ref_model_inputs.update({"output_hidden_states": output_hidden_states} if output_hidden_states else {})
...
class UniRGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            # if isinstance(model, str) and re.search(r"checkpoint-\d+$", model) and args.do_eval : # RGRPO EVAL Load  Traned Base Model
        ...
        if peft_config is not None:
            ...
            if hasattr(args, "lora_weight") and args.lora_weight and args.do_eval:
                print("Loading LoRA weights from", args.lora_weight)
        ...
        if isinstance(ref_model, str) and ref_model != 'None':# in RGRPO
        ...
        elif self.args.eval_strategy != "no":
    ...
    def _generate_and_score_completions(
        ...
        print("completion_ids len", completion_ids.shape)
        ...
        ###
        gold_answer =[  inp['answer'] for inp in inputs  ]
        ...
        for i, reward_func in enumerate(self.reward_funcs):
            ...
            if not self.model.training:
                reward_values = reward_per_func[:].detach().cpu().tolist()
                self._metrics[mode][f"rewards/{reward_func_name}"].extend(reward_values)#.append(reward_per_func[i].item())
            else:
                self._metrics[mode][f"rewards/{reward_func_name}"].append(reward_per_func[i].item())

        if not self.model.training:
            reward_values = rewards.detach().cpu().tolist()
            self._metrics[mode]["reward"].extend(reward_values)
            std_values = std_grouped_rewards.detach().cpu().tolist()
            self._metrics[mode]["reward_std"].extend(std_values)
        else:
        ...
        #print("should_evaluate?", self.control.should_evaluate)
        #if self.control.should_evaluate:
        os.makedirs(os.path.join(self.args.output_dir, "output_log"), exist_ok=True)
        save_path = os.path.join(self.args.output_dir, "output_log", "eval_completions.jsonl")
        with open(save_path, "a") as f:  # append mode!
            for p, c, r, a in zip(prompts_text, completions_text, rewards.tolist(),gold_answer):
                json.dump({
                    "prompt": p,
                    "completion": c,
                    "answer": a,
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys: Optional[list[str]] = None):
        ...
        print(self.control.should_evaluate)
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        if mode == "eval":
            ...
            print(f"[INFO] Saved eval metrics to {metrics_save_path}")
