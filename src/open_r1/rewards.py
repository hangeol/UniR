...
def grade_answer_sympy(given_answer: str, ground_truth: str) -> bool:
    ...
    if ground_truth_normalized is None or given_normalized is None:##fixed
        return False
...
def normalize_completions(completions):
    if (
...
def boxed_reward_fn_llama(completions, answer, **kwargs) -> list[float]:
    solution = answer
    responses = normalize_completions(completions)
    fast = False

    rewards = []
    for model_response , gt_answer in zip(responses, solution):
        #model_answer = extract_answer(model_response)
        model_answer = extract_xml_answer(model_response)
        print("- model_answer", model_answer)
        print("> gt_answer", gt_answer)
        if model_answer is None:
            r = 0.0  # Cannot even parse anything.
            is_correct = False
            # return {"formatted": False}, 0.0  # Cannot even parse anything.

        if isinstance(gt_answer, float) or isinstance(gt_answer, int):
            gt_answer = str(gt_answer)
        if isinstance(gt_answer, str) and model_answer is not None:
            is_correct = grade(model_answer, gt_answer, fast)
        elif isinstance(gt_answer, list) and model_answer is not None:
            is_correct = False
            for gt in gt_answer:
        ...
        if is_correct:
            ...
            # return {"formatted": True }, 0.0  # Formatted but wrong answer; no format reward to avoid hacking.
...
def boxed_reward_fn_qwen(completions, solution, **kwargs) -> list[float]:
    responses = normalize_completions(completions) 
    fast = False
...
def answer_tag_reward_fn(completions, answer,**kwargs):
    fast=False
    rewards = []
    # We are strict about format to evaluate our models.
    for model_response, gt_answer in zip(completions, answer):
        if "</think> <answer>" in model_response and "</answer>" in model_response:
            ...
            if isinstance(gt_answer, str):
                ...
                for gt in gt_answer:
                    is_correct |= grade(model_answer, gt, fast)
            if is_correct:
                rewards.append(float(1.0))  # Correctness reward.
            else:
                rewards.append(float(0))
        else:
            rewards.append(float(0))
...
def extract_number_answer(text: str) -> int:
    """
    Extracts the last numeric value (including floats with units like $17.00, 17%, 17m)
    and returns it as an integer.
    """
    def extract_last_number(s):
        # Match numbers with optional unit prefixes/suffixes
        # matches = re.findall(r"[-+]?\d+(?:\.\d+)?", s)
        matches = re.findall(r"[-+]?(?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d+)?", s)
        if matches:
            # return int(float(matches[-1]))  # Convert float string to int
            cleaned = matches[-1].replace(",", "")
            return int(float(cleaned))
        return None

    # Step 1: Try inside <answer>...</answer>
    answer_block = re.search(r"<answer>([\s\S]*?)</answer>", text, re.IGNORECASE)
    if answer_block:
        val = extract_last_number(answer_block.group(1))
        if val is not None:
            return val

    # Step 2: Fallback to whole text
    val = extract_last_number(text)
    return val if val is not None else -1

def extract_hash_answer(text: str) -> str | None:
    if "####" not in text:
        return None
    return int(float(text.split("####")[1].strip().replace(",", "")))

def rulebased_correct_reward_func(prompts, completions, answer, **kwargs) -> list[float]:
    responses = normalize_completions(completions) #[completion[0]['content'] for completion in completions]
    extracted_responses = [extract_number_answer(r) for r in responses]
    answer = [extract_hash_answer(r) for r in answer]
    print(f"!!Extracted: {extracted_responses}")
