...
class CombinedModel_qwen(Qwen2ForCausalLM):
    def __init__(self, config, model: PreTrainedModel, ref_model: PreTrainedModel):
    ...
    def __call__(self, *args, ref_model_inputs=None, **kwargs):
        return self.forward(*args, ref_model_inputs=ref_model_inputs, **kwargs)
    ...
    def forward(
        ...
        ref_model_inputs = None,
        **kwargs
    ) -> Union[torch.Tensor, CausalLMOutputWithPast]:
        model_out = self.model(input_ids=input_ids, attention_mask=attention_mask, **kwargs)

        # if get logits
        if ref_model_inputs is None:
            with torch.no_grad():
                ref_out = self.ref_model(input_ids=input_ids, attention_mask=attention_mask, **kwargs)
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            # UniR start
            ref_model_inputs = self.prepare_inputs_for_generation(input_ids, **ref_model_kwargs)
            # prepare variable output controls (note: some models won't accept all output controls)
            ref_model_inputs.update({"output_attentions": output_attentions} if output_attentions else {})
            ref_model_inputs.update({"output_hidden_states": True})
            

            if is_prefill:
                outputs = self(**model_inputs, return_dict=True, ref_model_inputs = ref_model_inputs)
                is_prefill = False
            else:
                outputs = model_forward(**model_inputs, return_dict=True, ref_model_inputs = ref_model_inputs)
            ...
            ref_model_kwargs = self._update_model_kwargs_for_generation(
                outputs,
                model_kwargs,
            )
            # end
...
class CombinedModel_llama(LlamaForCausalLM):
    def __init__(self, config, model: PreTrainedModel, ref_model: PreTrainedModel):
        super().__init__(config)
        self.lm_head = None
        self.model = model
    ...
    def from_pretrained(cls, path, *args, **kwargs):
        ...
        model = AutoModelForCausalLM.from_pretrained(os.path.join(path, "model"))
        ref_model = AutoModelForCausalLM.from_pretrained(args.ref_name_or_path,trust_remote_code=True)
        return cls(config, model=model, ref_model=ref_model)
    ...
    def forward(
        ...
        **kwargs
        ...
        # if get logits
        if ref_model_inputs is None:
            # combined_logits = model_out.logits
...
class UniRGRPOTrainer(Trainer):
    ...
    def __init__(
        self,
        model: Union[str, PreTrainedModel],
        ref_model: Union[str, PreTrainedModel],#UniR 
        use_unir: bool, #UniR
        reward_funcs: Union[RewardFunc, list[RewardFunc]],
        ...
        if isinstance(ref_model, str) and ref_model != 'None':# in UNiR
            ...
            if "Qwen2.5-VL" in ref_model:
                from transformers import Qwen2_5_VLForConditionalGeneration, AutoProcessor
                self.ref_vl_processing = AutoProcessor.from_pretrained(ref_model,trust_remote_code=True)
                self.ref_model = Qwen2_5_VLForConditionalGeneration.from_pretrained(ref_model, torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2",device_map=None,trust_remote_code=True)
            else:
                self.ref_model = AutoModelForCausalLM.from_pretrained(self.ref_name_or_path, **model_init_kwargs)            
    
            for param in self.ref_model.parameters():
                param.requires_grad = False
        #reference model in basic GRPO to calculate KL divergence(from original code)
        elif self.beta == 0.0:
        ...
        if isinstance(ref_model, str) and ref_model in ["Qwen/Qwen2.5-7B-Instruct", "Qwen/Qwen2.5-14B-Instruct"]:
            #if len(processing_class) != model.config.vocab_size :
            print("Mismatch detected! Fixing it...")
            model.resize_token_embeddings(len(processing_class))
            self.ref_model.resize_token_embeddings(len(processing_class))

        if self.use_unir == True:
            print("UniR GRPO start",model)
            ...
            if "llama" in model_type:
                ...
                print("qwen reasoning module load")
                model = CombinedModel_qwen(config = model.config, model = model, ref_model = self.ref_model )
        ...
        if self.use_vllm:
            ...
            if args.temperature == 0 :
                ...
                print("Using sampling decoding with temperature",self.temperature)
                generation_kwargs = {
                    "max_new_tokens": self.max_completion_length,
                    "do_sample": True,
                    "pad_token_id": processing_class.pad_token_id,
    ...
    def _generate_and_score_completions(
        ...
        #UniR modification start
        if hasattr(self, "ref_vl_processing"):
            for p in prompts:
                if isinstance(p[1]["content"], str):
                    p[1]["content"] = [{"text": p[1]["content"]}, {"image": "file://image.png"}]
            prompts_text = self.ref_vl_processing.apply_chat_template(prompts, tokenizer= False, add_generation_prompt = True)


            prompt_inputs = self.ref_vl_processing(text = prompts_text, images = inputs[0]["image"], return_tensors="pt", padding=True,torch_dtype=torch.bfloat16 )
        ...
        if self.use_vllm:
            ...
            with unwrap_model_for_generation(
                ...
                #UniR modification start
                    if hasattr(self, "ref_vl_processing"):
                        print("VL model generation")
                        for k, v in prompt_inputs.items():
                            if isinstance(v, torch.Tensor) and v.dtype == torch.float32:
                                prompt_inputs[k] = v.to(dtype=torch.bfloat16)
        ...
        answers = [inp['answer'] for inp in inputs if 'answer' in inp]

        if self.first_step:
            os.makedirs(os.path.join(self.args.output_dir,"output_log"), exist_ok=True)
            save_path = os.path.join(self.args.output_dir,"output_log",  "completions.jsonl")
            write_mode = "w"
            self.first_step = False  # 다음부터는 append 모드로 바뀌도록 설정
        else:
            save_path = os.path.join(self.args.output_dir, "output_log", "completions.jsonl")
            write_mode = "a"
        with open(save_path, write_mode) as f:  # append mode!
            for p, c, r, a in zip(prompts_text, completions_text, rewards.tolist(),answers):
                json.dump({
                    "prompt": p,
                    "completion": c,
                    "answer": a,
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        os.makedirs(os.path.join(self.args.output_dir, "output_log"), exist_ok=True)
        metrics_save_path = os.path.join(self.args.output_dir, "output_log", "metrics.jsonl")
